...
The script provided \verb|test.py| runs an experiment where first 20 rounds are made to determine the estimated optimal number of epochs for training each of the five networks. 
...
The only result that is not produced by the script but referenced in the report is the how the accuracy of the DCM was \textit{initially} estimated.
It was not included in the script, because the process is well documented in Section~\ref{sub::sec::digit_classification}, it is a minor result, and \textit{final} accuracy estimation is provided within the script.
...
The \verb|NumPy| package is used for this project, but \textit{only} for \verb|.csv| saving and \verb|pyplot| plotting, and not for networks- or statistics-related processing.

Finally, the \verb|test.py| script in its entirety was coarsely benchmarked, and the results are presented in Table~\ref{tab::script_metrics} below:
